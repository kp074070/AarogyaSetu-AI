...
function CustomerDashboard() {
  ...
  const [stats, setStats] = useState({ total: 0, upcoming: 0, completed: 0, cancelled: 0, nextAppointment: null });
  const [nearbyPHCs, setNearbyPHCs] = useState([]);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [aptStats, phcData, aptData] = await Promise.all([
          api.getAppointmentStats(),
          api.getNearbyPHCs(user?.state || 'Maharashtra', 4),
          api.getAppointments({ status: 'completed', limit: 3 }),
        ]);
        setStats(aptStats);
        setNearbyPHCs(phcData.phcs || []);
        setRecentVisits(aptData.appointments || []);
      } catch (err) {
    ...
    const handleBedUpdate = (data) => {
      setNearbyPHCs(prev => prev.map(p => p.phcId === data.phcId ? { ...p, availableBeds: data.availableBeds } : p));
    };
  ...
  if (loading) {
    return <div className="animate-fade-in" style={{ display: 'flex', justifyContent: 'center', padding: 60 }}>
    ...
    <div className="animate-fade-in customer-dashboard">
      ...
      <div className="customer-welcome-banner">
        <div className="welcome-content">
          <div className="welcome-greeting">
            <h2>Welcome back, <span className="text-primary">{user?.fullName?.split(' ')[0] || 'Patient'}</span> 👋</h2>
        ...
        <div className="welcome-decoration"><Heart size={64} className="welcome-icon" /></div>
      </div>

      {/* Quick Actions */}
      <div className="quick-actions-grid">
        <div className="quick-action-card" onClick={() => navigate('/customer/appointments')}>
          <div className="qa-icon qa-icon-blue"><CalendarCheck size={22} /></div>
      ...
      <div className="customer-grid">
        ...
        <div className="card animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
          <div className="card-header">
            ...
            <button className="btn btn-ghost btn-sm" onClick={() => navigate('/customer/appointments')}>View All <ChevronRight size={14} /></button>
          </div>
          {stats.nextAppointment ? (
            <div className="next-appointment-card">
              <div className="na-date">
                <div className="na-day">{new Date(stats.nextAppointment.date).getDate()}</div>
                <div className="na-month">{new Date(stats.nextAppointment.date).toLocaleString('default', { month: 'short' })}</div>
              </div>
              <div className="na-details">
                <div className="na-doctor">{stats.nextAppointment.doctor.name}</div>
                <div className="na-dept">{stats.nextAppointment.department}</div>
                <div className="na-meta">
                  <span><Clock size={12} /> {stats.nextAppointment.time}</span>
                  <span><MapPin size={12} /> {stats.nextAppointment.phcName}</span>
                </div>
              </div>
              <span className="status-badge info">Token #{stats.nextAppointment.tokenNumber}</span>
            </div>
          ) : (
            <div className="empty-state" style={{ padding: '30px 20px' }}><CalendarCheck size={32} /><p>No upcoming appointments</p></div>
          )}
          ...
          {recentVisits.length > 0 && (
            <div style={{ marginTop: 16 }}>
              ...
              {recentVisits.map(apt => (
                <div key={apt._id} className="recent-apt-item">
        ...
        <div className="card animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
          <div className="card-header">
            ...
            <button className="btn btn-ghost btn-sm" onClick={() => navigate('/customer/find-phc')}>See All <ChevronRight size={14} /></button>
          </div>
          <div className="nearby-phc-list">
            {nearbyPHCs.map((phc, idx) => (
              <div key={phc.phcId || idx} className="nearby-phc-item" style={{ animationDelay: `${idx * 0.05}s` }}>
                <div className="phc-item-left">
                  <div className={`phc-status-indicator ${phc.riskLevel}`}></div>
                  <div>
                    <div className="phc-item-name">{phc.name}</div>
                    <div className="phc-item-meta"><span><MapPin size={11} /> {phc.district}, {phc.state}</span></div>
                  </div>
                </div>
                <div className="phc-item-right">
                  <div className="phc-item-rating"><Star size={12} fill="var(--accent)" color="var(--accent)" /><span>{phc.rating}</span></div>
                  <div className="phc-item-beds"><BedDouble size={12} /><span>{phc.availableBeds} beds</span></div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="card health-advisory animate-fade-in-up" style={{ animationDelay: '0.35s' }}>
        ...
        <div className="advisory-cards">
          <div className="advisory-item advisory-monsoon"><div className="advisory-icon">🌧️</div><div><strong>Monsoon Season Alert</strong><p>Increased risk of Dengue, Malaria, and Waterborne diseases. Use mosquito nets and drink boiled water.</p></div></div>
          <div className="advisory-item advisory-vaccination"><div className="advisory-icon">💉</div><div><strong>Vaccination Drive</strong><p>Free COVID-19 booster shots available at all PHCs. Carry your Aadhaar card for registration.</p></div></div>
          <div className="advisory-item advisory-checkup"><div className="advisory-icon">🩺</div><div><strong>Annual Health Checkup</strong><p>Free annual health screening for citizens above 30 years. Visit your nearest PHC.</p></div></div>
        </div>
      </div>
    </div>
  );
